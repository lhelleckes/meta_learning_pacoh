...
class MAMLRegression(RegressionModelMetaLearned):
    def __init__(
        ...
        self.meta_train_data = [
            self._prepare_data_per_task(train_x, train_y, flatten_y=False)
            for train_x, train_y in meta_train_data
        ]

        self.nn = NeuralNetwork(
            self.input_dim, self.output_dim, layer_sizes=layer_sizes
        )
    ...
    def meta_fit(self, valid_tuples=None, verbose=True, log_period=500, n_iter=None):
        ...
        for itr in range(1, n_iter + 1):
            task_batch_indices = self.rds_numpy.choice(
                len(self.meta_train_data), size=self.task_batch_size
            )
            meta_train_tuples = [
                self.meta_train_data[idx] for idx in task_batch_indices
            ]
    ...
    def predict(
        self, context_x, context_y, test_x, return_tensor=False, num_steps_eval=None
    ):
        """
        ...
        adapted_params = self._eval_steps(
            context_x, context_y, num_steps_eval=num_steps_eval
        )
        with torch.no_grad():
            y_pred = self.nn.forward_parametrized(test_x, adapted_params)
            y_pred = (
                y_pred * torch.Tensor(self.y_std).float()[None, :]
                + torch.Tensor(self.y_mean).float()[None, :]
            )

            y_pred2 = self.nn.forward_parametrized(test_x, self.initial_params)
            y_pred2 = (
                y_pred2 * torch.Tensor(self.y_std).float()[None, :]
                + torch.Tensor(self.y_mean).float()[None, :]
            )
    ...
    def eval(self, context_x, context_y, test_x, test_y, num_steps_eval=None):
        ...
        rmse = torch.mean(
            torch.sum(torch.pow(y_pred - test_y_tensor, 2), dim=-1)
        ).sqrt()
    ...
    def eval_datasets(self, test_tuples, **kwargs):
        ...
        rmse_list = [
            self.eval(*test_data_tuple, **kwargs) for test_data_tuple in test_tuples
        ]
    ...
    def _eval_steps(self, x_data, y_data, num_steps_eval=None):
        ...
        for step in range(num_steps_eval):
            mse_loss = self.loss_fn(
                self.nn.forward_parametrized(x_data, temp_params), y_data
            )
...
if __name__ == "__main__":
    ...
    meta_test_data = dataset.generate_meta_test_data(
        n_tasks=1000, n_samples_context=5, n_samples_test=100
    )

    meta_learner = MAMLRegression(
        meta_train_data, task_batch_size=10, num_iter_fit=10000
    )
