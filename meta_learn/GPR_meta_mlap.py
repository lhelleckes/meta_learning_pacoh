...
class GPRegressionMetaLearnedPAC(RegressionModelMetaLearned):
    def __init__(
        ...
        self.meta_train_params.append(
            {"params": posterior_params, "lr": posterior_lr_multiplier * lr}
        )
    ...
    def meta_fit(
        ...
        """
        assert (
            eval_period % log_period == 0
        ), "eval_period should be multiple of log_period"
    ...
    def predict(
        self, context_x, context_y, test_x, n_iter_meta_test=3000, return_density=False
    ):
        """
        ...
        task_dict = self._meta_test_inference(
            [(context_x, context_y)],
            verbose=True,
            log_period=500,
    ...
    def eval_datasets(self, test_tuples, n_iter_meta_test=3000, **kwargs):
        ...
        task_dicts = self._meta_test_inference(
            context_tuples, verbose=True, log_period=500, n_iter=n_iter_meta_test
        )
        ...
        for task_dict, test_tuple in zip(task_dicts, test_tuples):
            ...
            test_x_tensor = (
                torch.from_numpy(self._normalize_data(X=test_x, Y=None))
                .float()
                .to(device)
            )
            ...
            ll_list.append(
                torch.mean(pred_dist.log_prob(test_y_tensor) / test_y_tensor.shape[0])
                .cpu()
                .item()
            )
            rmse_list.append(
                torch.mean(torch.pow(pred_dist.mean - test_y_tensor, 2))
                .sqrt()
                .cpu()
                .item()
            )
    ...
    def _setup_task_dicts(self, train_data_tuples):
        ...
        for train_x, train_y in train_data_tuples:
            ...
            task_dict["gp_model"].variational_distribution.variational_mean.data.copy_(
                init_mean
            )

            prior_covar = covar_module(x_tensor)
            init_chol_covar = torch.cholesky(
                prior_covar + 1e-3 * torch.eye(prior_covar.shape[0])
            )
            task_dict[
                "gp_model"
    ...
    def _meta_test_inference(
        self, context_tuples, n_iter=5000, lr=1e-2, log_period=100, verbose=False
    ):
    ...
    def _setup_meta_train_step(
        ...
        def _task_pac_bounds(
            ...
            for task_dict in task_dicts:
                ...
                # likelihood
                avg_ll = torch.mean(
                    self.likelihood.expected_log_prob(task_dict["train_y"], posterior)
                )
                ...
                kl_inner = task_kl_weight * torch.mean(
                    _kl_divergence_safe(posterior.expand((self.svi_batch_size,)), prior)
                )
        ...
        def _meta_complexity(prior_param_sample, meta_kl_weight=1.0):
            ...
            n = torch.tensor(self.n_tasks, dtype=torch.float32)
            return torch.sqrt(
                meta_kl_weight * outer_kl
                + math.log(2.0)
                + torch.log(n)
                - torch.log(self.delta) / (2 * (n - 1))
            )
    ...
    def prior_mean(self, x, n_hyperposterior_samples=1000):
        ...
        x_data_tiled = np.tile(
            x.reshape(1, x.shape[0], 1), (n_hyperposterior_samples, 1, 1)
        )
    ...
    def _aggregate_gp_priors(self, prior_param_sample, jitter=1e-5):
        ...
        def covar_module(x):
            ...
            cov_loc = torch.mean(
                torch.bmm(residual.unsqueeze(2), residual.unsqueeze(1)), axis=0
            )
...
def _kl_divergence_safe(posterior, prior):
    for jitter_eps in [1e-6, 1e-5, 1e-4]:
        try:
            ...
            warnings.warn(
                "added jitter of %s to the diagonal posterior and prior covariance"
                % str(jitter_eps)
            )
    return torch.distributions.kl.kl_divergence(posterior, prior)
...
if __name__ == "__main__":
    ...
    gp_model = GPRegressionMetaLearnedPAC(
        meta_train_data,
        num_iter_fit=20000,
        task_kl_weight=1.0,
        meta_kl_weight=1e-5,
    ...
    for i in range(2):
        itrs = 0
        gp_model.meta_fit(
            valid_tuples=meta_test_data[:10],
            log_period=500,
            eval_period=10000,
            n_iter=10000,
        )
        ...
        for j in range(1):
            ...
            pred_mean, pred_std = gp_model.predict(
                x_context, t_context, x_plot, n_iter_meta_test=3000
            )
