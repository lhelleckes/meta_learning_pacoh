...
class GPRegressionMetaLearnedSVGD(RegressionModelMetaLearned):
    def __init__(
        ...
        """
        super().__init__(normalize_data, random_seed, one_hot_idx)
...
if __name__ == "__main__":
    ...
    meta_test_data = data_sim.generate_meta_test_data(
        n_tasks=5, n_samples_context=40, n_samples_test=160
    )
    meta_train_data = [
        (context_x, context_y) for context_x, context_y, _, _ in meta_test_data
    ]
    ...
    for prior_factor in [1e-3]:
        gp_model = GPRegressionMetaLearnedSVGD(
            meta_train_data,
            num_iter_fit=2000,
        ...
        for i in range(10):
            ...
            plt.title(
                "GPR meta SVGD (prior-factor =  %.4f) itrs = %i" % (prior_factor, itrs)
            )
            plt.show()
