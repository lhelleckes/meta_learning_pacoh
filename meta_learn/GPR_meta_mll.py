...
class GPRegressionMetaLearned(RegressionModelMetaLearned):
    def __init__(
        ...
        self.lr_params, self.weight_decay, self.feature_dim = (
            lr_params,
            weight_decay,
            feature_dim,
        )
        ...
        self.shared_parameters.append(
            {"params": self.likelihood.parameters(), "lr": self.lr_params}
        )
    ...
    def _setup_gp_prior(
        ...
        if learning_mode in ["learn_kernel", "both"]:
            self.shared_parameters.append(
                {"params": self.covar_module.hyperparameters(), "lr": self.lr_params}
            )

        if learning_mode in ["learn_mean", "both"] and self.mean_module is not None:
            self.shared_parameters.append(
                {"params": self.mean_module.hyperparameters(), "lr": self.lr_params}
            )
...
if __name__ == "__main__":
    ...
    meta_test_data = data_sim.generate_meta_test_data(
        n_tasks=50, n_samples_context=10, n_samples_test=160
    )
    ...
    for weight_decay in [0.8, 0.5, 0.4, 0.3, 0.2, 0.1]:
        gp_model = GPRegressionMetaLearned(
            meta_train_data,
            num_iter_fit=20000,
        ...
        for i in range(1):
            gp_model.meta_fit(
                valid_tuples=meta_test_data, log_period=1000, n_iter=20000
            )
            itrs += 20000
            ...
            ucb, lcb = gp_model.confidence_intervals(
                x_context, t_context, x_plot, confidence=0.9
            )
            ...
            plt.title(
                "GPR meta mll (weight-decay =  %.4f) itrs = %i" % (weight_decay, itrs)
            )
            plt.show()
