    ...
    def train_reg(config, reporter):
        ...
        with gpytorch.settings.max_cg_iterations(300):
            ...
            for i in range(config["num_iter_fit"] // log_period):
                loss, diagnostics_dict = model.meta_fit(
                    verbose=False,
                    log_period=1000,
                    eval_period=100000,
                    n_iter=log_period,
    ...
    if args.load_analysis:
        ...
        space = {
            "task_kl_weight": hp.loguniform(
                "task_kl_weight", math.log(5e-2), math.log(1e0)
            ),
            "meta_kl_weight": hp.loguniform(
                "meta_kl_weight", math.log(1e-7), math.log(1e0)
            ),
            "lr": hp.loguniform("lr", math.log(1e-4), math.log(1e-3)),
            "lr_decay": hp.loguniform("lr_decay", math.log(0.92), math.log(0.97)),
            "posterior_lr_multiplier": hp.loguniform(
                "posterior_lr_multiplier", math.log(1e0), math.log(10.0)
            ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="whether to resume checkpointed tune session",
    )
    parser.add_argument(
