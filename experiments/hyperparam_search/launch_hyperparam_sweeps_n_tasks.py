...
flags.DEFINE_string(
    "algos", default="map,vi,svgd", help="specifies which dataset to use"
)
flags.DEFINE_string(
    "metric", default="test_rmse", help="specifies which metric to optimize"
)
...
flags.DEFINE_boolean(
    "load_analysis",
    default=False,
    help="whether to load the analysis from existing dirs",
)
flags.DEFINE_boolean(
    "resume", default=False, help="whether to resume checkpointed tune session"
)

FLAGS = flags.FLAGS


algo_map_dict = {"map": meta_mll_hparam, "vi": meta_vi_hparam, "svgd": meta_svgd_hparam}


def main(argv):
    assert FLAGS.dataset in ["sin", "cauchy"]

    hparam_search_modules = [
        algo_map_dict[algo_str] for algo_str in FLAGS.algos.split(",")
    ]
    ...
    for hparam_search_module in hparam_search_modules:
        exp_config = {
            "dataset": [
                "%s_%i" % (FLAGS.dataset, n_tasks)
                for n_tasks in reversed([5, 10, 20, 40, 80, 160, 320])
            ],
            "covar_module": ["NN"],
        ...
        command_list += generate_launch_commands(
            hparam_search_module, exp_config, check_flags=False
        )
    ...
    if FLAGS.cluster:
        ...
        answer = input(
            "About to run %i compute jobs in a for loop. Proceed? [yes/no]\n"
            % len(command_list)
        )
        if answer == "yes":
