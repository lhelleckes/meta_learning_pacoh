...
DATASETS = [
    "sin_20"
]  # ['sin_20', 'cauchy_20', 'swissfel', 'physionet_0', 'physionet_1']
...
for dataset in DATASETS:
    ...
    model_map = GPRegressionMetaLearned(
        meta_train_data,
        num_iter_fit=1000,
        covar_module="NN",
        mean_module="NN",
    ...
    model_svgd = GPRegressionMetaLearnedSVGD(
        meta_train_data,
        num_iter_fit=1000,
        num_particles=5,
        covar_module="NN",
    ...
    model_vi = GPRegressionMetaLearnedVI(
        meta_train_data,
        num_iter_fit=1000,
        covar_module="NN",
        mean_module="NN",
    ...
    model_mlap = GPRegressionMetaLearnedPAC(
        meta_train_data,
        num_iter_fit=1000,
        svi_batch_size=5,
        covar_module="NN",
    ...
    for gp_model, model_name in [
        (model_map, "pacoh_map"),
        (model_svgd, "pacoh_svgd"),
        (model_vi, "pacoh_vi"),
        (model_mlap, "mlap"),
    ]:
        ...
        compute_times_meta_train[dataset][model_name] = (
            np.mean(durations_meta_train) / 10.0,
            np.std(durations_meta_train) / 10.0,
        )
        compute_times_meta_test[dataset][model_name] = (
            np.mean(durations_meta_test) / 10.0,
            np.std(durations_meta_test) / 10.0,
        )
    ...
    # meta_train iter
    durations_mean, durations_std = list(
        zip(*[compute_times_meta_train[dataset][method] for method in METHODS])
    )
    ax[0].bar(
        range(len(labels)),
        durations_mean,
    ...
    durations_mean, durations_std = list(
        zip(*[compute_times_meta_test[dataset][method] for method in METHODS])
    )
    ax[1].bar(
        range(len(labels)),
        durations_mean,
